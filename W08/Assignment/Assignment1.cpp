...
void findCustomerByID(Connection* conn) {
    ...
    ResultSet* rs = stmt->executeQuery("SELECT * FROM dbs211_customers WHERE customer_id = " + to_string(customerID));
...
void customerReport(Connection* conn) {
    ...
    if (rs->next()) {
        ...
        ResultSet* orderRS = stmt->executeQuery("SELECT * FROM dbs211_orders WHERE customer_id = " + to_string(customerID));
        // Output order details

        // Query purchased product information
        ResultSet* productRS = stmt->executeQuery("SELECT * FROM dbs211_ products WHERE product_id IN "
...
void addCustomer(Connection* conn) {
    ...
    Statement* stmt = conn->createStatement();
    int rowCount = stmt->executeUpdate("INSERT INTO dbs211_customers (customer_name, customer_email) "
...
void removeCustomer(Connection* conn) {
    ...
    Statement* stmt = conn->createStatement();
    int rowCount = stmt->executeUpdate("DELETE FROM dbs211_customers WHERE customer_id = " + to_string(customerID));
